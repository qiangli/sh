...
func newMapImporter(base string, fallback types.Importer) *mapImporter {
	return &mapImporter{base: base, packages: map[string]*types.Package{}, files: map[string][]string{}, fallback: fallback}
}
...
func (m *mapImporter) add(spec PackageSpec, pkg *types.Package) error {
	...
	sort.Strings(names)
	m.packages[spec.Path] = pkg
	m.files[spec.Path] = names
	return nil
}

// checkDependency parses and type-checks one explicit package in lexical
// filename order and registers it. Parse and type diagnostics are returned
// together, attributed by file position exactly as the main package's are;
// nothing is converted or executed.
func (m *mapImporter) checkDependency(fset *token.FileSet, spec PackageSpec, goVersion string) ErrorList {
	...
	pkg, err := config.Check(spec.Path, fset, files, nil)
