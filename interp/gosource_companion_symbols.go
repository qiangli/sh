...
// interpreted body becomes a trampoline; a package variable is refused.
func (r *Runner) bashPPGoSourceCompanionTrampolines(files []string) ([]bashPPCompanionTrampoline, error) {
	...
	for _, file := range files {
		...
		if err != nil {
			return nil, fmt.Errorf("gosource: assembly companion %s: %w", filepath.Base(file), err)
		}
	...
	for _, name := range names {
		for _, stmt := range r.bashPPGoSourceFile.Stmts {
			...
			case *syntax.BashPPDecl:
				...
				return nil, fmt.Errorf("gosource: assembly companion references original variable %s: interpreted package state cannot be shared with compiled code", name)
			default:
				continue
			}
			break
