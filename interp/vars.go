...
func (r *Runner) setVarString(name, value string) {
	if base, idx, ok := splitArrayRef(name); ok && syntax.ValidName(base) {
		r.setVarWithIndex(r.lookupVar(base), base, r.arrayTargetIndex(idx), expand.Variable{Set: true, Kind: expand.String, Str: value})
		return
	}
...
func (r *Runner) setVar(name string, vr expand.Variable) {
	if base, idx, ok := splitArrayRef(name); ok && syntax.ValidName(base) && vr.Kind == expand.String {
		r.setVarWithIndex(r.lookupVar(base), base, r.arrayTargetIndex(idx), vr)
		return
	}
