...
func (r *Runner) bashPPSprint162StoredBridgeScalar(expr syntax.BashPPExpr, expected syntax.BashPPTypeExpr) (any, *bashPPCollectionMeta, bool, error) {
...
func (r *Runner) bashPPSprint162CollectionInterfaceDynamicType(cell *bashPPCell) syntax.BashPPTypeExpr {
	if cell == nil || cell.vr.Kind != expand.String {
		return nil
	}
	kind := cell.scalarKind
	if kind == 0 {
		kind = r.bashPPScalarFromCell(cell).value.Kind()
	}
	name := bashPPDefaultScalarTypeName(kind)
	if name == "" {
		return nil
	}
...
func bashPPSprint162MapPart(text string) string {
	return strconv.Itoa(len(text)) + ":" + text
}

func bashPPSprint162PointerMapValue(pointer *bashPPPointer) string {
	if pointer == nil {
		return "nil"
	}
	var path strings.Builder
	for _, step := range pointer.path {
		path.WriteString(bashPPSprint162MapPart(step.field))
		path.WriteByte('/')
...
func (r *Runner) bashPPSprint162MapKey(value any, meta *bashPPCollectionMeta, typ syntax.BashPPTypeExpr) (bashPPMapKey, bool, error) {
	if meta != nil && meta.interfaceValue != nil {
		...
		key, nonreflexive, err := r.bashPPSprint162MapKey(inner, innerMeta, dynamic)
	...
	case *syntax.BashPPNamedType:
		...
		case "string":
			...
			return bashPPMapKey{typ: typeName, value: "s" + bashPPSprint162MapPart(text)}, false, nil
		case "bool":
	...
	case *syntax.BashPPPointerType:
		...
		return bashPPMapKey{typ: typeName, value: bashPPSprint162PointerMapValue(pointer)}, false, nil
	...
	case *syntax.BashPPCollectionType:
		...
		for i, item := range sequence {
			...
			part, nonreflexive, err := r.bashPPSprint162MapKey(item, child, x.Element)
			if err != nil {
				return bashPPMapKey{}, false, err
			}
			if nonreflexive {
			...
			encoded.WriteString(bashPPSprint162MapPart(part.typ))
			encoded.WriteString(bashPPSprint162MapPart(part.value))
		}
	...
	case *syntax.BashPPStructType:
		...
		for _, field := range bashPPFlatFields(x.Fields) {
			...
			part, nonreflexive, err := r.bashPPSprint162MapKey(fieldValue, child, field.typ)
			if err != nil {
				return bashPPMapKey{}, false, err
			}
			if nonreflexive {
			...
			encoded.WriteString(bashPPSprint162MapPart(field.name))
			encoded.WriteString(bashPPSprint162MapPart(part.typ))
			encoded.WriteString(bashPPSprint162MapPart(part.value))
		}
...
func (r *Runner) bashPPSprint162MapLookup(meta *bashPPCollectionMeta, value any, valueMeta *bashPPCollectionMeta, typ syntax.BashPPTypeExpr) (string, *bashPPMapEntry, bool, error) {
	key, nonreflexive, err := r.bashPPSprint162MapKey(value, valueMeta, typ)
...
func (r *Runner) bashPPSprint162MapDelete(mapping map[string]any, meta *bashPPCollectionMeta, value any, valueMeta *bashPPCollectionMeta, typ syntax.BashPPTypeExpr) error {
	key, nonreflexive, err := r.bashPPSprint162MapKey(value, valueMeta, typ)
	if err != nil || nonreflexive || meta == nil {
		return err
	}
...
func (r *Runner) bashPPSprint162MapStore(mapping map[string]any, meta *bashPPCollectionMeta, value any, valueMeta *bashPPCollectionMeta, typ syntax.BashPPTypeExpr, element any, child *bashPPCollectionMeta) (string, error) {
	key, nonreflexive, err := r.bashPPSprint162MapKey(value, valueMeta, typ)
	if err != nil {
		return "", err
	}
...
func bashPPSprint162MapEntries(meta *bashPPCollectionMeta) []*bashPPMapEntry {
	if meta == nil {
		return nil
	}
	bashPPStorageMu.RLock()
...
func bashPPSprint162MapIterationEntries(meta *bashPPCollectionMeta) []*bashPPMapEntry {
	return bashPPSprint162MapEntries(meta)
}

func bashPPSprint162MapHasTypedKeys(meta *bashPPCollectionMeta) bool {
	if meta == nil {
		return false
	}
	bashPPStorageMu.RLock()
...
func bashPPSprint162MapEntryTable(meta *bashPPCollectionMeta) map[bashPPMapKey]*bashPPMapEntry {
	if meta == nil {
		return nil
	}
	bashPPStorageMu.RLock()
...
func bashPPSprint162MapEntryValue(mapping map[string]any, meta *bashPPCollectionMeta, storage string) (any, *bashPPCollectionMeta, bool) {
	bashPPStorageMu.RLock()
...
func bashPPSprint162MapEntryValueFound(mapping map[string]any, meta *bashPPCollectionMeta, storage string, found bool) (any, *bashPPCollectionMeta) {
	if !found {
		return nil, nil
	}
	value, child, _ := bashPPSprint162MapEntryValue(mapping, meta, storage)
	return value, child
}

func bashPPSprint162MapClear(mapping map[string]any, meta *bashPPCollectionMeta) {
	bashPPStorageMu.Lock()
	clear(mapping)
	if meta != nil {
		clear(meta.mapping)
