...
| `time` | `time.ex` and `time_NE.ex`: F 13, 15, 18; `time.ex`: U 7, 8, 11, 14, 16, 17, 23, 26, 29; US 3; warning 20 | Source defect fixed: `time -p` uses LC_ALL > LC_NUMERIC > LANG radix precedence for the carried Profile D `de_DE.iso88591` locale. No behavior is claimed for arbitrary host locales. TP7/8 remain explicitly not portable. Covered by `TestTimePosixLocaleRadix`. |
