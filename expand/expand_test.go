...
func TestParamExpAnchoredEmptyPattern(t *testing.T) {
	cfg := &Config{Env: ListEnviron("var=blah")}
	tests := []struct {
		src  string
		want string
	}{
	...
	// Test empty variable
	cfg2 := &Config{Env: ListEnviron("var=")}
	word := parseWord(t, `${var/#/x}`)
	got, err := Literal(cfg2, word)
	if err != nil {
		t.Fatalf("empty var: did not want error, got %v", err)
	}
	if want := "x"; got != want {
		t.Fatalf("empty var: wanted %q, got %q", want, got)
	}
}
